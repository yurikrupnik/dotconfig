...
pub async fn run_docker_compose_new(
    compose_file: &Path,
    detach: bool,
    extra_args: &[String],
) -> anyhow::Result<()> {
    ...
    match action {
        ComposeAction::Up { file, detach, args, mam } => {
            ...
            if detach {
                ...
                println!("Running in detached mode {detach:?}");
            };
            let compose_file = resolve_compose_file(file)?;

            let mut args: Vec<String> = vec![
                // "-f".into(),
                // compose_file.to_ascii_lowercase().to_owned(),
            ];

            // let mut cmd: Vec<&str> = vec!["up"];
            // if detach {
            //     cmd.push("-d");
            // }
            // args.extend(
            //     cmd.into_iter().map(|s| s.to_string())
            // );

            let cmd = vec!["up"];
            // let cmd = vec!["up"];
            // let result = run_docker_compose_new(&compose_file, detach, &args).await?;
            // let result = run_docker_compose_new(cmd, &compose_file, detach, &args).await;
            let result = run_docker_compose(&cmd, &compose_file, detach, &args).await?;
            // works detach - testing with flag
            // let result = run_docker_compose(&["up", "-d"], &compose_file, detach, &args).await;
